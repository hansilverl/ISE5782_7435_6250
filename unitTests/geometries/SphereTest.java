...
class SphereTest {
    ...
    void testFindIntersection() {
        ...
        result = sphere.findIntersection(new Ray(new Point(1, 0, 1),
                new Vector(1, 1, 1)));
        assertNull(result, "Wrong number of points");

        // TC18: Ray starts after sphere (0 points)
        result = sphere.findIntersection(new Ray(new Point(1, 0, 2),
                new Vector(1, 1, 1)));
        assertNull(result, "Wrong number of points");

        // **** Group: Ray's line is tangent to the sphere (all tests 0 points)
        // TC19: Ray starts before the tangent point
        result = sphere.findIntersection(new Ray(new Point(2, -1, -1),
                new Vector(0, 1, 1)));
        assertNull(result, "Wrong number of points");

        // TC20: Ray starts at the tangent point
        result = sphere.findIntersection(new Ray(new Point(2, 0, 0),
                new Vector(0, 1, 1)));
        assertNull(result, "Wrong number of points");
        ...
        result = sphere.findIntersection(new Ray(new Point(3, 0, -1),
                new Vector(0, 0, 1)));
        assertNull(result, "Wrong number of points");
    }
}
