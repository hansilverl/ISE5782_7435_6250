...
public class IntegrationTest {
    ...
    private int countIntersectionsCameraGeometry(Camera camera, int nX, int nY, Intersectable geometry){
        ...
        for (int i = 0; i < nX; i++) {
            for (int j = 0; j < nY; j++) {
                intersections = geometry.findIntersection(camera.constructRay(nX, nY, j, i));
    ...
    public void CameraRayTriangleIntegration() {
        ...
        // TC01:  1 point
        assertEquals(1, countIntersectionsCameraGeometry(cam,3,3,
                new Triangle(new Point(1, -1, -2), new Point(-1, -1, -2), new Point(0, 1, -2))),"Bad number of intersections");
